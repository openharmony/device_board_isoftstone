...
int set_wifi_custom_mac_address(char *addr_str, size_t count)
{
	if ((count > ADDR_STR_LEN) || addr_parse(ADDR_FMT_STR, addr_str, 1)) {
		ADDR_MGT_ERR("Format wrong.");
		return -EINVAL;
	}

	return set_addr_by_name(TYPE_USER, ADDR_FMT_STR, addr_str, "wifi");
}
EXPORT_SYMBOL_GPL(set_wifi_custom_mac_address);
