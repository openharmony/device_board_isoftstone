		...
		struct disp_layer_config *config)
{
	...
	memcpy(&layer_inner->info.screen_win,
		&config->info.screen_win, sizeof(struct disp_rect));
			...
			unsigned int plane_num)
{
	...
	dev = scrtc->crtc.dev;
	memset(&data, 0, sizeof(struct disp_manager_data));
	memset(layer_data, 0,
		16 * sizeof(struct disp_layer_config_data));
...
static int sunxi_drm_smooth_set_state(struct drm_device *dev)
{
	...
	if (!conn->state) {
		conn->state
			= kzalloc(sizeof(struct drm_connector_state),
							GFP_KERNEL);
		if (!conn->state) {
	...
	if (!crtc->state) {
		crtc->state =
			kzalloc(sizeof(struct drm_crtc_state), GFP_KERNEL);
		if (!crtc->state) {
...
static int sunxi_drm_platform_probe(struct platform_device *pdev)
{
	...
	memset(&sunxi_drv_prv, 0, sizeof(struct sunxi_drm_private));
