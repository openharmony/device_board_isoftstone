			...
			struct disp_rect *rect)
{
	...
	memcpy(&info->window, rect, sizeof(struct disp_rect));
			...
			struct disp_rect *rect)
{
	...
	if (rect)
		memcpy(rect, &info->window, sizeof(struct disp_rect));
...
/*get clk of de*/
	de_drv->mclk = devm_clk_get(&pdev->dev, "clk_de0");
	if (IS_ERR(de_drv->mclk)) {
		DRM_ERROR("fail to get clk for de\n");
		return -EINVAL;
	}

	de_drv->mclk_bus = devm_clk_get(&pdev->dev, "clk_bus_de0");
	if (IS_ERR(de_drv->mclk)) {
...
int sunxi_de_module_init(void)
{
	...
	de_drv = kzalloc(sizeof(struct sunxi_de_drv), GFP_KERNEL);
	if (!de_drv) {
