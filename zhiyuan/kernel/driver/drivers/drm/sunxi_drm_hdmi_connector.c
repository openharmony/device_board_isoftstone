		...
		struct sunxi_hdmi_work_mode *work_mode)
{
	...
	memcpy(&hdmi_conn->work_mode,
		work_mode, sizeof(struct sunxi_hdmi_work_mode));
...
/*set hdmi booting mode as default drm display mode*/
	...
	memcpy(tmp_mode, display_mode, sizeof(struct drm_display_mode));
...
sunxi_drm_connector_hdmi_create(struct drm_device *dev, int conn_id)
{
	...
	sunxi_hdmi_conn = kzalloc(sizeof(struct sunxi_drm_hdmi_connector),
				GFP_KERNEL);
	if (!sunxi_hdmi_conn) {
	...
	memcpy(&sunxi_hdmi_conn->work_mode,
		...
		sizeof(struct sunxi_hdmi_work_mode));
