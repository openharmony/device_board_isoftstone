...
static int sunxi_mmc_finalize_request(struct sunxi_mmc_host *host)
{
	...
	if (data && data->flags & MMC_DATA_WRITE && (host->sunxi_mmc_hw_wbusy_wait))
...
void sunxi_mmc_do_shutdown_com(struct platform_device *pdev)
{
	...
	if (mmc == NULL) {
		SM_ERR(&pdev->dev, "%s: mmc is NULL\n", __func__);
		goto out;
	}

	host = mmc_priv(mmc);
	if (host == NULL) {
		SM_ERR(&pdev->dev, "%s: host is NULL\n", __func__);
		goto out;
	}

	SM_INFO(mmc_dev(mmc), "try to disable cache\n");
	mmc_claim_host(mmc);
	err = mmc_flush_cache(mmc->card);
	mmc_release_host(mmc);
	if (err) {
		SM_ERR(mmc_dev(mmc), "disable cache failed\n");
/*not release host to not allow android to read/write after shutdown */
	...
	SM_DBG(mmc_dev(mmc), "%s: claim host\n", __func__);
	mmc_claim_host(mmc);

	do {
		if (mmc_send_status(mmc->card, &status) != 0) {
			SM_ERR(mmc_dev(mmc), "%s: send status failed\n",
				__func__);
	/*
	*err_out;
	*not release host to not allow android to read/write after shutdown
	*/
	...
	err = mmc_go_idle(mmc);
	if (err) {
		SM_ERR(mmc_dev(mmc), "%s: mmc_go_idle err\n", __func__);
	/*
	*err_out;
	//not release host to not allow android to read/write after shutdown
	*/
...
/*sd can support cmd1,so not send cmd1 */
	...
	if (err) {
		SM_ERR(mmc_dev(mmc), "%s: first mmc_send_op_cond err\n",
			__func__);
	/*
	*err_out;
	*not release host to not allow android to read/write after shutdown
	*/
	...
	if (err) {
		SM_ERR(mmc_dev(mmc), "%s: mmc_send_op_cond err\n",
			__func__);
	/*err_out;
	*not release host to not allow android to read/write after shutdown
	*/
...
out:
	SM_INFO(mmc_dev(mmc), "%s: mmc shutdown exit..ok\n", __func__);
}
#endif
...
#if IS_ENABLED(CONFIG_AW_MMC_V4P5X)
