	...
	if (scan_req != local->int_scan_req) {
		...
		if (true == aborted) {
			if (HdfWifiEventScanDone(get_krn_netdev(), 1) < 0) {
				...
				printk(KERN_WARNING "HdfWifiEventScanDone 1 ok!\n");
			}
		} else {
			if (HdfWifiEventScanDone(get_krn_netdev(), 0) < 0) {
				printk(KERN_WARNING "HdfWifiEventScanDone 0 Failed!\n");
			} else {
					     ...
					     unsigned long *next_delay)
{
	...
	case NL80211_BSS_CHAN_WIDTH_20:
		...
		break;
	default:
		local->scan_chandef.width = scan_req->scan_width;
		break;
	}
